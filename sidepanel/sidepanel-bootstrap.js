...
const appState = window.SidepanelAppState.createSidepanelAppState({
      ...
      upiRedeemCdkeyStatusAutoRefreshTimer: undefined,
      upiRedeemCdkeyStatusRefreshInFlight: undefined,
      flowCapabilityRegistry: undefined,
});

with (appState.createScope()) {
  function pickScopeValues(names = []) {
    return Object.fromEntries(names.map((name) => [name, eval(name)]));
  }
  latestState = null;
  ...
  function syncStepDefinitionsForMode(plusModeEnabled = false, plusPaymentMethodOrOptions = {}, maybeOptions = {}) {
    ...
    rebuildStepDefinitionState(Boolean(stepDefinitionState.plusModeEnabled), {
      ...
      registrationFreeRoute: options.registrationFreeRoute ?? currentRegistrationFreeRoute,
    });
