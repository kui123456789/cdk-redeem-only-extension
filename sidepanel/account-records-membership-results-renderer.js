(function attachSidepanelAccountRecordsMembershipResultsRenderer(globalScope) {
  function createAccountRecordsMembershipResultsRenderer(context = {}) {
    ...
    const summarizeRows = typeof context.summarizeMembershipViewModelRows === 'function' ? context.summarizeMembershipViewModelRows : () => ({ free: 0, 'upi-plus': 0, 'ideal-plus': 0 });
      ...
      return `
        ...
        <div class="upi-membership-check-list" data-upi-membership-list="${escapeHtml(group)}">
          ${groupRows.length ? groupRows.map((row) => {
            ...
            const deleteChannel = group === 'paid-ideal' ? 'ideal' : group === 'paid-upi' ? 'upi' : normalizeText(row.redeemChannel || row.channel);
    ...
    function renderUpiCredentialMembershipCheckResults() {
      ...
      const redeemAllUpiEmailSet = new Set(redeemableUpiFreeRows.slice(0, redeemUpiNowCount).map((row) => normalizeEmail(row.email)).filter(Boolean));
      const redeemAllIdealNowCount = Math.min(redeemableIdealFreeRows.filter((row) => !redeemAllUpiEmailSet.has(normalizeEmail(row.email))).length, availableIdealCdkeyCount);
      ...
      container.innerHTML = `
        ...
        <div class="upi-membership-check-section" data-upi-membership-section="free">
          <div class="upi-membership-check-head upi-membership-section-head"><span class="upi-membership-section-title">Free 组</span><span class="upi-membership-section-meta">${escapeHtml(String(summary.free))} 个账号 · 待兑换 ${escapeHtml(String(freeRows.length))} · 可兑换 ${escapeHtml(String(redeemableFreeRows.length))} · UPI候选 ${escapeHtml(String(redeemableUpiFreeRows.length))} · IDEAL候选 ${escapeHtml(String(redeemableIdealFreeRows.length))}${upiDailyLimitBlockedFreeCount ? ` · UPI日限 ${escapeHtml(String(upiDailyLimitBlockedFreeCount))}` : ''} · 失败 ${escapeHtml(String(failedRows.length))} · 封存 ${escapeHtml(String(lockedRedeemCount))} · 缺 AT ${escapeHtml(String(missingAtCount))}</span></div>
          ...
          <div class="upi-membership-check-actions">
            ...
            <button class="btn btn-ghost btn-xs" type="button" data-upi-membership-redeem-free data-upi-membership-redeem-channel="upi" ${redeemUpiNowCount && !membershipBusy ? '' : 'disabled'} title="${escapeHtml(`UPI 可用 CDK ${availableUpiCdkeyCount}；UPI 候选 ${redeemableUpiFreeRows.length}；总可兑换 ${redeemableFreeRows.length}；普通 UPI 失败不会禁用 UPI${upiDailyLimitBlockedFreeCount ? `；${upiDailyLimitBlockedFreeCount} 个账号明确返回今日提交次数上限，已转 IDEAL 候选` : ''}`)}">一键兑换 UPI(${escapeHtml(String(redeemUpiNowCount))}/${escapeHtml(String(redeemableUpiFreeRows.length))})</button><button class="btn btn-ghost btn-xs" type="button" data-upi-membership-redeem-free data-upi-membership-redeem-channel="ideal" ${Math.min(redeemableIdealFreeRows.length, availableIdealCdkeyCount) && !membershipBusy ? '' : 'disabled'} title="${escapeHtml(`IDEAL 可用 CDK ${availableIdealCdkeyCount}；IDEAL 候选 ${redeemableIdealFreeRows.length}；总可兑换 ${redeemableFreeRows.length}${idealFailureBlockedFreeCount ? `；${idealFailureBlockedFreeCount} 个账号 IDEAL 已失败满 ${redeemChannelFailureLimit} 次或已封存` : ''}`)}">一键兑换 IDEAL(${escapeHtml(String(Math.min(redeemableIdealFreeRows.length, availableIdealCdkeyCount)))}/${escapeHtml(String(redeemableIdealFreeRows.length))})</button><button class="btn btn-primary btn-xs" type="button" data-upi-membership-redeem-all ${redeemUpiNowCount + redeemAllIdealNowCount && !membershipBusy ? '' : 'disabled'} title="${escapeHtml(`先兑换 UPI ${redeemUpiNowCount}/${redeemableFreeRows.length}，完成后再兑换剩余 IDEAL ${redeemAllIdealNowCount}/${redeemableFreeRows.length}；只有明确返回今日提交次数上限的账号会跳过 UPI 并进入 IDEAL 候选`)}">一键兑换全部(${escapeHtml(String(redeemUpiNowCount + redeemAllIdealNowCount))}/${escapeHtml(String(redeemableFreeRows.length))})</button>
