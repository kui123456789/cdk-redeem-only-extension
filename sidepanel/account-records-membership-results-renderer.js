      ...
      container.innerHTML = `
        ...
        <div class="upi-membership-check-section" data-upi-membership-section="free">
          ...
          <div class="upi-membership-check-actions">
            ...
            <button class="btn btn-ghost btn-xs" type="button" data-upi-membership-fill-free-at ${missingAtCount && !flags.mutatingBusy ? '' : 'disabled'}>一键补充 AT(${escapeHtml(String(missingAtCount))})</button><button class="btn btn-ghost btn-xs" type="button" data-upi-membership-refresh-invalid-at ${identifyPlusCount && !flags.mutatingBusy ? '' : 'disabled'} title="检查现有 AT；仅在明确失效时重新登录获取并验证新 AT">检查并刷新 AT(${escapeHtml(String(identifyPlusCount))})</button><button class="btn btn-primary btn-xs" type="button" data-upi-membership-identify-free-plus ${identifyPlusCount && !flags.mutatingBusy ? '' : 'disabled'}>一键识别 Plus(${escapeHtml(String(identifyPlusCount))})</button><button class="btn btn-ghost btn-xs" type="button" data-upi-membership-refresh-all-email-statuses ${identifyPlusCount && !membershipBusy ? '' : 'disabled'} title="${escapeHtml(missingAtCount ? `按已保存 AT 检测 Free/失败组邮箱会员状态；当前可刷新 ${identifyPlusCount} 个，${missingAtCount} 个缺少 AT。` : '按已保存 AT 检测 Free/失败组邮箱会员状态')}">一键刷新所有邮箱状态(${escapeHtml(String(identifyPlusCount))})</button>
