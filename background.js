...
const statePatchHelpers = self.MultiPageBackgroundStatePatchHelpers.createStatePatchHelpers({
  registrationEmailStateHelpers,
  runtimeStateHelpers,
...
function splitCustomEmailPoolEntrySource(value = []) {
  return customEmailPoolStateRegistry.splitCustomEmailPoolEntrySource(value);
}

function normalizeCustomEmailPool(value = []) {
  return customEmailPoolStateRegistry.normalizeCustomEmailPool(value);
}

function normalizeCustomMailProviderPoolEntries(value = []) {
  return customEmailPoolStateRegistry.normalizeCustomMailProviderPoolEntries(value);
}

function parseCustomEmailPoolEntryForState(value = '') {
  return customEmailPoolStateRegistry.parseCustomEmailPoolEntryForState(value);
}

function normalizeCustomEmailVerificationUrlForState(value = '') {
  return customEmailPoolStateRegistry.normalizeCustomEmailVerificationUrlForState(value);
}

function normalizeCustomEmailPoolTrialEligibilityStatus(value = '') {
  return customEmailPoolStateRegistry.normalizeCustomEmailPoolTrialEligibilityStatus(value);
}

function isCustomEmailPoolEntryTrialIneligible(entry = {}) {
  return customEmailPoolStateRegistry.isCustomEmailPoolEntryTrialIneligible(entry);
}

function isCustomEmailPoolEntryAvailable(entry = {}) {
  return customEmailPoolStateRegistry.isCustomEmailPoolEntryAvailable(entry);
}

function maskCustomEmailPoolAccessToken(token = '') {
  return customEmailPoolStateRegistry.maskCustomEmailPoolAccessToken(token);
}

function normalizeCustomEmailPoolEntryObjects(value = []) {
  return customEmailPoolStateRegistry.normalizeCustomEmailPoolEntryObjects(value);
}

function isCustomEmailPoolGenerator(stateOrValue = {}) {
  return customEmailPoolStateRegistry.isCustomEmailPoolGenerator(stateOrValue);
}

function getCustomEmailPool(state = {}) {
  return customEmailPoolStateRegistry.getCustomEmailPool(state);
}

function getCustomEmailPoolEntries(state = {}) {
  return customEmailPoolStateRegistry.getCustomEmailPoolEntries(state);
}

function getCustomEmailPoolCredentialForEmail(state = {}, email = '') {
  return customEmailPoolStateRegistry.getCustomEmailPoolCredentialForEmail(state, email);
}
...
async function markCurrentCustomEmailPoolEntryUsed(state = {}, options = {}) {
  return customEmailPoolStateRegistry.markCurrentCustomEmailPoolEntryUsed(state, options);
}
...
const customEmailPoolStateRegistry = self.MultiPageCustomEmailPoolState?.createCustomEmailPoolState?.({
  addLog,
  broadcastDataUpdate,
  getCustomEmailPoolGeneratorName: () => (typeof CUSTOM_EMAIL_POOL_GENERATOR === 'string'
    ? CUSTOM_EMAIL_POOL_GENERATOR
    : 'custom-pool'),
  getState,
  normalizeCustomEmailVerificationUrl,
...
function getCustomEmailPoolEmailForRun(state = {}, targetRun = 1) {
  return customEmailPoolStateRegistry.getCustomEmailPoolEmailForRun(state, targetRun);
}

function getCustomMailProviderPool(state = {}) {
  return customEmailPoolStateRegistry.getCustomMailProviderPool(state);
}

function getCustomMailProviderPoolEmailForRun(state = {}, targetRun = 1) {
  return customEmailPoolStateRegistry.getCustomMailProviderPoolEmailForRun(state, targetRun);
}
...
function getSettingsTransferManager() {
  if (!settingsTransferManager) {
    settingsTransferManager = self.MultiPageBackgroundSettingsTransfer.createSettingsTransfer({
      chromeApi: chrome,
      settingsExportFilenamePrefix: SETTINGS_EXPORT_FILENAME_PREFIX,
      settingsExportSchemaVersion: SETTINGS_EXPORT_SCHEMA_VERSION,
      defaultLocalCpaJsonRelativeAuthDir: DEFAULT_LOCAL_CPA_JSON_RELATIVE_AUTH_DIR,
      storageKeys: {
        membershipResults: UPI_CREDENTIAL_MEMBERSHIP_CHECK_RESULTS_STORAGE_KEY,
        credentialBackups: UPI_ACCOUNT_CREDENTIAL_BACKUPS_STORAGE_KEY,
        accountRunHistory: ACCOUNT_RUN_HISTORY_STORAGE_KEY,
      },
      persistentAliasStateKeys: PERSISTENT_ALIAS_STATE_KEYS,
      defaultState: DEFAULT_STATE,
      defaultRegistrationEmailState: DEFAULT_REGISTRATION_EMAIL_STATE,
      accountRunHistoryHelpers,
      normalizeBooleanMap,
      normalizeIcloudAliasCacheList,
      toNormalizedEmailSet,
      getPersistedSettings,
...
function normalizeLocalCpaJsonPluginDir(rawValue = '') {
  return getSettingsTransferManager().normalizeLocalCpaJsonPluginDir(rawValue);
}

function normalizeLocalCpaJsonRelativeAuthDir(rawValue = '') {
  return getSettingsTransferManager().normalizeLocalCpaJsonRelativeAuthDir(rawValue);
}

function buildSettingsExportFilename(date = new Date()) {
  return getSettingsTransferManager().buildSettingsExportFilename(date);
}

function normalizeSettingsRuntimeObject(value, fallback = {}) {
  return getSettingsTransferManager().normalizeSettingsRuntimeObject(value, fallback);
}

function normalizeSettingsRuntimeEmail(value = '') {
  return getSettingsTransferManager().normalizeSettingsRuntimeEmail(value);
}

function normalizeSettingsRuntimeMembershipResults(value = null) {
  return getSettingsTransferManager().normalizeSettingsRuntimeMembershipResults(value);
}

function normalizeSettingsRuntimeCredentialBackups(value = null) {
  return getSettingsTransferManager().normalizeSettingsRuntimeCredentialBackups(value);
}

function normalizeSettingsRuntimeAccountRunHistory(value = null) {
  return getSettingsTransferManager().normalizeSettingsRuntimeAccountRunHistory(value);
}

function normalizeSettingsRuntimeAliasState(value = null) {
  return getSettingsTransferManager().normalizeSettingsRuntimeAliasState(value);
}

function getSettingsRuntimeDataForExport() {
  return getSettingsTransferManager().getSettingsRuntimeDataForExport();
}
...
function importSettingsBundle(configBundle) {
  return getSettingsTransferManager().importSettingsBundle(configBundle);
}
...
const signupExecutorRegistry = self.MultiPageBackgroundSignupExecutorRegistry.createSignupExecutorRegistry({
  ...
  STEP6_MAX_ATTEMPTS,
  STEP6_REGISTRATION_SUCCESS_WAIT_MS,
...
const step9Executor = signupExecutorRegistry.executors.step9;
