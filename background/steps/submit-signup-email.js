(function attachBackgroundStep2(root, factory) {
  ...
  function createStep2Executor(deps = {}) {
    ...
    async function openSignupAuthEntryForStep2(reasonMessage = '') {
      if (reasonMessage) {
        await addLog(reasonMessage, 'warn');
      }
      const entry = await ensureSignupAuthEntryPageReady(2);
      const state = String(entry?.result?.state || '').trim();
      if (state) {
        await addLog(`步骤 2：认证入口页状态 ${state}，继续注册流程。`);
      } else {
        await addLog('步骤 2：认证入口页已打开，继续注册流程。');
      }
      return entry.tabId;
    }
    ...
    async function executeSignupEmailEntry(state) {
      ...
      if (await shouldForceAuthEntryRetry(signupTabId)) {
        try {
          signupTabId = await openSignupAuthEntryForStep2('步骤 2：检测到当前 ChatGPT 首页未进入邮箱注册入口，正在打开认证入口页再提交邮箱。');
        } catch (entryError) {
      ...
      if (step2Result?.error) {
        ...
        if (isSignupEntryUnavailableErrorMessage(errorMessage)) {
          signupTabId = await openSignupAuthEntryForStep2('步骤 2：未找到邮箱输入入口，正在打开认证入口页后重试一次...');
          ...
          signupTabId = await openSignupAuthEntryForStep2('步骤 2：注册入口页通信超时，正在打开认证入口页并重试提交邮箱...');
          step2Result = await submitSignupEmail(resolvedEmail, {
            timeoutMs: 45000,
            retryDelayMs: 700,
            logMessage: '步骤 2：认证入口页已打开，正在重新提交邮箱...',
